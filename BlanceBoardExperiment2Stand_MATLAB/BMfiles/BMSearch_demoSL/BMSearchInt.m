...
% randshiftx=RandSample([-rangex:rangex],[marker,types]);
% randshifty=RandSample([-rangey:rangey],[marker,types]);
for i=1:types
...
% BM_Upr(:,2,:,i)=-(BM3D(:,3,:)-(max(NormilizedY(:))+min(NormilizedY(:)))/2+...
%     repmat(randshifty(:,i),[1,1,frame])+repmat(squeeze(BM_Upr(:,2,1,i)),[1,1,frame]))*amplifier;
% BM_Inv(:,2,:,i)=(BM3D(:,3,:)-(max(NormilizedY(:))+min(NormilizedY(:)))/2+...
...
% BM_Upr(:,1,:,i)=(BM3D(:,1,:)*cos(proangle(i))+BM3D(:,2,:)*sin(proangle(i))+...
%     repmat(randshiftx(:,i),[1,1,frame])-repmat(squeeze(BM_Upr(:,1,1,i)),[1,1,frame]))*amplifier;
% BM_Inv(:,1,:,i)=(BM3D(:,1,:)*cos(proangle(i))+BM3D(:,2,:)*sin(proangle(i))+...
...
[x,y]=pol2cart(linspace(0,pi*2,max(setsize)+1),RadiusOffset);
x(end)=[]; y(end)=[];
...
cuecond=[1:4]; %cue condition: upright tar vs. nontar, inverted tar vs. nontar
...
tarpos=randperm(max(setsize));
initframe=RandSample(1:frame,[stimseq(trial,2),1]);
BM_Show=zeros(marker,2,frame,stimseq(trial,2));
switch stimseq(trial,1)
    case {1} % Upright target with inverted distractors
        BM_Show(:,:,:,1)=BM_Upr(:,:,:,RandSample(1:types));
        BM_Show(:,:,:,2:end)=BM_Inv(:,:,:,RandSample(1:types,[stimseq(trial,2)-1,1]));
    case {2} % Inverted distractors only
        BM_Show(:,:,:,:)=BM_Inv(:,:,:,RandSample(1:types,[stimseq(trial,2),1]));
    case {3} % Inverted target with upright distractors
        BM_Show(:,:,:,1)=BM_Inv(:,:,:,RandSample(1:types));
        BM_Show(:,:,:,2:end)=BM_Upr(:,:,:,RandSample(1:types,[stimseq(trial,2)-1,1]));
    case {4} % Upright distractors only
        BM_Show(:,:,:,:)=BM_Upr(:,:,:,RandSample(1:types,[stimseq(trial,2),1]));
end
trialtime=GetSecs;
while ~(touch==1 && (keyCode(Key1) || keyCode(Key2) || keyCode(escapeKey)))
    Screen('DrawTexture',windowPtr,defaultwin);
for i=1:stimseq(trial,2)
    Screen('DrawDots',windowPtr,squeeze(BM_Show(:,:,initframe(i),i))',DotSize,white,[centerx+x(tarpos(i)) centery+y(tarpos(i))],1);
end
...
save(datafile,'results','stimseq','ntrials','proangle','cuecond','setsize');
